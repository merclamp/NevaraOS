...
Четыре базовых этапа готовы и проверены в QEMU:
...
Linux x86_64 (read, write, open, close, lseek, getpid, mkdir, ...), возвращающий
отрицательный errno при ошибке. Вход через инструкцию `syscall` из ring 3
появится вместе с userspace; пока диспетчер вызывается напрямую.
